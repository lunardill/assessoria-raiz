...
- O próprio lead recebe mensagem automática no WhatsApp dele (script `mensagemLeadWhatsapp.gs`, mesmo Apps Script): 3 mensagens curtas em sequência com respiro entre elas, nome do lead capitalizado automaticamente, volume baixo (3-5 leads/semana). Mensagem fica genérica sobre a loja (sem nome específico), porque o nome da loja hoje só é identificado manualmente por quem atende, olhando o Instagram — não vem em nenhum campo do formulário.
- Os 4 scripts de automação de lead (e-mail, WhatsApp grupo, Meetime, WhatsApp lead) agora têm retry automático (até 3 tentativas) além do alerta de falha por e-mail, adicionado depois de uma falha real registrada em 18/08/2026.

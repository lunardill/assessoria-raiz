...
5. **Oportunidade da proposta** — pilares do Metodo RAIZ (ver `_contexto/metodo-raiz-comercial.md`) que fazem mais sentido pras dores levantadas, cada um ligado a uma dor especifica. Incluir tambem uma recomendacao sobre os entregaveis modulares (Google Meu Negocio, producao de criativos presencial) com o porque, seguindo a secao "Entregaveis modulares" do playbook — sempre como sugestao pro Lucas/Matheus validar, nunca como decisao fechada. Treinamento comercial entra por padrao em praticamente toda proposta, nao precisa de justificativa especial. Pra producao de criativos presencial, so sugerir se o lead for de Caxias do Sul, Rio Branco do Sul ou Itaperucu; fora disso, so mencionar se o usuario confirmar que tem terceirizado na regiao
